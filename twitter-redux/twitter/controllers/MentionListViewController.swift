...
class MentionListViewController: TweetsViewController {
    ...
    override func loadTimeline() {
        ...
        TwitterClient.sharedInstance.mentionsTimelineWithParams(params, completion: { (tweets, error) -> () in
            if (error == nil) {
                ...
                MBProgressHUD.hideHUDForView(self.view, animated: true)
                self.refreshControl.endRefreshing()
                TSMessage.showNotificationWithTitle("Error loading mentions timeline", type: TSMessageNotificationType.Error)
            }
        })
    }
